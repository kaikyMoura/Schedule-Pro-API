...
export class StaffAvailabilityService {
  ...
  /**
   * Retrieves all StaffAvailability objects from the database.
   *
   ...
   */
  async findMany(
    spec?: Specification<StaffAvailability>,
    options?: {
      where?: Prisma.StaffAvailabilityWhereInput;
      skip?: number;
      take?: number;
      include?: Prisma.StaffAvailabilityInclude;
      orderBy?: Prisma.StaffAvailabilityOrderByWithRelationInput;
    },
  ): Promise<StaffAvailability[]> {
    return await this.staffAvailabilityRepository.findMany({
   ...
   */
  async findById(id: string): Promise<StaffAvailability> {
    const staffAvailability = await this.staffAvailabilityRepository.findUnique(
      {
        where: { id: id },
      },
    );
