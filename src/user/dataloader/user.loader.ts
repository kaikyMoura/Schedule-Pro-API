...
import { CustomerIdsInAppointmentSpec } from 'src/appointment/specs/customer-id-in.spec';
import { StaffIdsInAppointmentSpec } from 'src/appointment/specs/staff-id-in.spec';
...
import { WithUsersIdsSpec } from 'src/user/specs/withs-users-ids.spec';
  ...
  createStaffAppointmentsLoader(): DataLoader<string, AppointmentType[]> {
    return new DataLoader<string, AppointmentType[]>(
      async (staffIds: string[]) => {
        const appointments = await this.appointmentService.findMany(
          new StaffIdsInAppointmentSpec(staffIds),
          {
            include: {
              service: true,
              customer: true,
